...
namespace SpaceCore.Framework
{
    public class CustomCraftingRecipe : CraftingRecipe
    {
        ...
        public override void drawMenuView(SpriteBatch b, int x, int y, float layerDepth = 0.88F, bool shadow = true)
        {
            ...
            Utility.drawWithShadow(b, recipe.IconTexture, new Vector2(x, y), recipe.IconSubrect ?? new Rectangle( 0, 0, recipe.IconTexture.Width, recipe.IconTexture.Height ), Color.White, 0f, Vector2.Zero, 4f, flipped: false, layerDepth);
        }
        ...
        public override void drawRecipeDescription(SpriteBatch b, Vector2 position, int width, IList<Item> additional_crafting_items)
        {
            ...
            b.Draw(Game1.staminaRect, new Rectangle((int)position.X + 8, (int)position.Y + lineExpansion + 64 + 4 + recipe.Ingredients.Length * 36, width - 32, 2), Game1.textColor * 0.35f);
            Utility.drawTextWithShadow(b, Game1.parseText(recipe.Description, Game1.smallFont, width - 8), Game1.smallFont, position + new Vector2(0f, 76 + recipe.Ingredients.Length * 36 + lineExpansion), Game1.textColor * 0.75f);
        }
