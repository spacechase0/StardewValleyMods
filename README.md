...
`Phases`                   | Determines how many days it takes to reach each corresponding growth sprite on the crop.png. Crops must have 5 phases if it regrows, but can have 2-6 if it is a one time harvest. The numbers in phases refer to how many days it takes for the plant to reach that phase. Seeds do not have a phase. Ex. [3, 1, 2, 2, 2, 3] This crop takes 3 days to reach the first growth sprite, 13 days to reach maturity, and does not regrow.
`RegrowthPhase`            | If your plant is a one time harvest set this to `-1`. If a positive number, this designates the amount of days it takes for the crop to regrow. The regrowth phase sprite must take up the last position on crop.png. Crops cannot regrow and have colors at the same time.
...
`IsColored`                   | _(optional)_ Set this value to `true` if your product is colored. Any object that comes from a colored crop must also be colored. Any object with `IsColored` set to `true` must also have a `color.png` (see above) included in the object subfolder.
`Recipe`                      | Set to `null`.
