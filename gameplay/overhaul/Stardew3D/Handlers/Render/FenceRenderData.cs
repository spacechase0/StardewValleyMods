...
public class FenceRenderData : RenderData<FenceRenderer>
{
        ...
        : base( ctx, parent)
    {
        ...
        {
            calcDrawing(ctx.WorldBatch);
        }
    }

    private void calcDrawing(RenderBatcher batch)
    {
        oldDrawSum = Parent.Object.getDrawSum();
        for (int i = 0; i < 4; ++i)
        {
            instances[i] = -1;
            ...
            {
                if (!(oldDrawSum is 10 or 100 or 110 && i is Game1.up or Game1.down) &&
                    !(oldDrawSum is 500 or 1000 or 1500 && i is Game1.left or Game1.right))
                {
                    if (oldDrawSum == 0)
                    {
                        genericId += $"/Gate";
            ...
            if (!batch.HasInstancedVerticesData(genericId))
            {
                ...
                batch.AddInstancedVerticesData(genericId, [data]);
            }
        ...
        if (Parent.Object.isGate.Value)
            oldDrawSum += 10000 * (1 + Parent.Object.gatePosition.Value);
    }
    ...
    public override void Update(RenderContext ctx)
    {
        ...
        if (oldDrawSum != Parent.Object.getDrawSum() + ( Parent.Object.isGate.Value ? (10000 * (1 + Parent.Object.gatePosition.Value)) : 0))
            calcDrawing(ctx.WorldBatch);
