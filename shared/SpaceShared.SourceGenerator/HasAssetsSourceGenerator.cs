...
{
    [Generator]
    public class HasAssetsSourceGenerator : IIncrementalGenerator
    {
        private record struct GenerationData(string? Namespace, string Name, string AssetInstancesType);

        public void Initialize(IncrementalGeneratorInitializationContext ctx)
        {
            var prov = ctx.SyntaxProvider.CreateSyntaxProvider(
                static (s, _) => (s is ClassDeclarationSyntax classDecl && classDecl.AttributeLists.Any(al => al.Attributes.Any(a => a.Name.ToString().StartsWith("HasAssets<") || a.Name.ToString().StartsWith("SpaceShared.Attributes.HasAssets<"))) && classDecl.BaseList.Types.Any()),//static t => (t is SimpleBaseTypeSyntax simpleType && simpleType.Type is SimpleNameSyntax simpleName && ( simpleName.Identifier.ValueText?.StartsWith( "BaseMod<" ) ?? false )))),
                static (ctx, _) =>
                {
                    ...
                    var attr = classSym.GetAttributes().First(a => a.AttributeClass.Name == "HasAssetsAttribute").AttributeClass; ;
...
{{
    private static PerScreen< {data.AssetInstancesType} > _assetInstances = new( () => new {data.AssetInstancesType}(Instance) );
    public static {data.AssetInstancesType} Assets {{ get {{ return _assetInstances.Value; }} }}
}}
";

            ctx.AddSource($"{data.Namespace}.{data.Name}.WithAssets.g.cs", code);
        }
    }
}
