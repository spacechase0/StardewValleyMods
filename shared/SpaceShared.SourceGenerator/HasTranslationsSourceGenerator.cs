...
{
    [Generator]
    public class HasTranslationsSourceGenerator : IIncrementalGenerator
    {
        ...
        public void Initialize(IncrementalGeneratorInitializationContext ctx)
        {
            var prov = ctx.SyntaxProvider.CreateSyntaxProvider(
                static (s, _) => (s is ClassDeclarationSyntax classDecl && classDecl.AttributeLists.Any( al => al.Attributes.Any( a => a.Name.ToString().StartsWith("HasTranslations") || a.Name.ToString().StartsWith("SpaceShared.Attributes.HasTranslations") ) ) && classDecl.BaseList.Types.Any()),//static t => (t is SimpleBaseTypeSyntax simpleType && simpleType.Type is SimpleNameSyntax simpleName && ( simpleName.Identifier.ValueText?.StartsWith( "BaseMod<" ) ?? false )))),
                static (ctx, _) =>
                {
                    ...
                    var attr = classSym.GetAttributes().First(a => a.AttributeClass.Name == "HasTranslationsAttribute").AttributeClass;
