    ...
    internal static class TrueSight
    {
        ...
        *********/
        ...
        public static void DrawOverWorld(SpriteBatch spriteBatch)
        {
            foreach (var pair in Game1.currentLocation.netObjects.Pairs)
            {
                ...
                {
                    if (obj.itemId.Value is "343" or "450")
                    {
                ...
                {
                    doDraw = TrueSight.GetMineStoneDrop(obj.itemId.Value, (int)pos.X, (int)pos.Y, Game1.player, (Game1.currentLocation as MineShaft));
                }

                if (obj.itemId.Value == "590")
                ...
                if (doDraw != "-1")
                {
                    ...
                    else
                    {
                        ...
                        {
                            drawObj = new SObject(new Vector2(0, 0), doDraw);
                ...
                for (int ix = 0; ix < Game1.currentLocation.Map.Layers[0].LayerWidth; ++ix)
                {
                    for (int iy = 0; iy < Game1.currentLocation.Map.Layers[0].LayerWidth; ++iy)
                    {
                        if (il.IsBuriedNutLocation(new Point(ix, iy)) && !Game1.netWorldState.Value.FoundBuriedNuts.Contains($"{il.NameOrUniqueName}_{ix}_{iy}"))
                        {
                            ...
                            {
                                drawObj = new SObject(new Vector2(0, 0), "73");
        ...
        public static string GetMineStoneDrop(string tileIndexOfStone, int x, int y, Farmer player, MineShaft mine)
        {
            ...
            if (mine.characters.Count == 0)
                num3 += 0.04;
            if (!mine.ladderHasSpawned && !mine.mustKillAllMonstersToAdvance() && (stonesLeftOnThisLevel == 0 || r.NextDouble() < num3))
                return "-2";
        ...
        *********/
        ...
        private static string GetBaseMineStoneDrop(string indexOfStone, int x, int y, Farmer player, Random r, MineShaft mine)
        {
            ...
            if (player.hasMagnifyingGlass && r.NextDouble() < 3.0 / 400.0)
            {
                ...
                if (unseenSecretNote != null)
                    ret = unseenSecretNote.itemId.Value;
            }
            return ret;
        }
        ...
        private static string GetArtifactSpotDrop(int x, int y, Farmer player)
        {
            ...
            foreach (KeyValuePair<string, StardewValley.GameData.Objects.ObjectData> keyValuePair in Game1.objectData)
            {
                string[] strArray1 = keyValuePair.Value.Name.Split('/');
            ...
            else
            {
                ...
                while (index1 < strArray.Length)
                {
                    if (random.NextDouble() <= Convert.ToDouble(strArray[index1 + 1]))
                    {
                        ...
                        if (Game1.objectData.TryGetValue(index2, out StardewValley.GameData.Objects.ObjectData? objData))
                        {
                            if (objData.Name.Split('/')[3].Contains("Arch") || index2 == "102")
                            {
                                if (index2 == "102" && Game1.netWorldState.Value.LostBooksFound >= 21)
                                    index2 = "770";
                                return index2;
                            }
                        }
