...
* Animations - You can animate textures by editing `"spacechase0.SpaceCore/TextureOverrides"`, which is a dictionary with the key being the ID of your animation, and the following information:
    ...
    * `TargetRect` - The rectangle in the target file you want to animate. Example: `{ "X": 32, "Y": 48, "Width": 16, "Height": 16 }`
        If this is not specified, anything from that texture will be overwritten with this animation.
    ...
    * `SourceSizeModifier` - A multiplier to apply to the size of the animation frames. Intended for use with `FullSheetMode` - if used otherwise, it applies after `SourceSizeOverride` (if specified).
    * `FullSheetMode` - `true` to use the bounds of the original texture for animation targetting, instead of `TargetRect`. Default `false`.
